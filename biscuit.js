...
if (process.argv.length < 3) {
    ...
    console.log(" --infile\t # extra urls to target, full url one on each line")
...
sitemapper.fetch(siteToCrawl).then(function (sites) {
    ...
    toWorkOn.forEach(function (item, index) {
        ...
        nettime({
            url: item, followRedirects: true, timeout: 30000,
...
function getExtraUrls(filename) {
    try {
